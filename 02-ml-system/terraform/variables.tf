variable "region" {
  description = "AWS region. The Academy Learner Lab only permits us-east-1."
  type        = string
  default     = "us-east-1"

  validation {
    condition     = var.region == "us-east-1"
    error_message = "This lab is validated only in us-east-1, the region AWS Academy allows."
  }
}
...
variable "training_image" {
  description = "Managed SageMaker built-in XGBoost image. Training and serving share it."
  type        = string
  default     = "683313688378.dkr.ecr.us-east-1.amazonaws.com/sagemaker-xgboost:1.7-1"
}

variable "instance_type" {
  description = "Instance type for training and for the endpoint variant."
  type        = string
  default     = "ml.m5.large"
}

variable "volume_size_in_gb" {
  description = "Training EBS volume. 30 GB is the size validated by the Academy preflight."
  type        = number
  default     = 30
}
...
variable "max_runtime_in_seconds" {
  ...
  validation {
    ...
    error_message = "Keep the stopping condition between 600 and 900 seconds (spec 9.4)."
  }
}
...
variable "deploy_serving" {
  description = <<-EOT
    Stage gate. False creates storage + training only; true additionally creates
    the Model, EndpointConfig and Endpoint. `make apply` flips it automatically
    once the training artifact has been located and proven to exist.
  EOT
  type        = bool
  default     = false
}
...
variable "model_artifact_uri" {
  description = <<-EOT
    ...
    scripts/wait_training.py and written to artifact.auto.tfvars.json. Never a
